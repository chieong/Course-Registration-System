...
public class TimetableValidator {
    ...
    public Instructor validateInstructorForExport(Integer staffId) throws TimetableValidationException {
        ...
        Instructor instructor = instructorRepository.findById(staffId)
                .orElseThrow(() -> new TimetableValidationException("Instructor not found with ID: " + staffId));
    ...
    public void validateTimetableData(TimetableData timetableData) throws TimetableValidationException {
        ...
        if (timetableData.getOwnerId() == null) {
            throw new TimetableValidationException("Owner ID is missing from timetable data");
        }

        if (timetableData.getUserType() == null) {
            throw new TimetableValidationException("User type is missing from timetable data");
        }

        if (timetableData.getSections() == null || timetableData.getSections().isEmpty()) {
            throw new TimetableValidationException("Timetable data has no section records");
        }
